...
def test_sphere(plot_scatter=True, show_grid=True, show_axis=True):
    def sphere(u, v):
        x = np.cos(u) * np.sin(v)
        y = np.sin(u) * np.sin(v)
        z = np.cos(v)
    ...
    plot_3d_points(samples, plot_scatter=plot_scatter, show_grid=show_grid, show_axis=show_axis)


def test_torus(plot_scatter=True, show_grid=True, show_axis=True):
...
def test_sine_wave(plot_scatter=True, show_grid=True, show_axis=True):
    def sine_wave(u, v):
        x = u
        y = v
        z = np.sin(u) * np.cos(v)
...
def test_saddle(plot_scatter=True, show_grid=True, show_axis=True):
    def saddle(u, v):
        x = u
        y = v
        z = u**2 - v**2
